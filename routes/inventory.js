...
module.exports = (app) => {
  ...
  /* ================================================= */
  /* ============ PUBLIC USER INVENTORY ============== */
  /* ================================================= */
  app.get("/api/inventory", async (req, res) => {
    try {

      const cars = await Inventory.find({
        status: "approved",
      }).sort({ createdAt: -1 });
  ...
  /* ================================================= */
  /* ============ SINGLE CAR (PUBLIC USER) ============ */
  /* ================================================= */
  app.get("/api/inventory/public/:id", async (req, res) => {
    try {

      const car = await Inventory.findOne({
        _id: req.params.id,
        status: "approved",
      });
      ...
      console.error("SINGLE INVENTORY ERROR:", err);
      res.status(500).json({ message: "Fetch failed" });
    }
  });
